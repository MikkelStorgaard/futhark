...
using Mono.Options;
#define DEBUG
FileStream runtime_file = null;
var do_warmup_run = false;
var num_runs = 1;
...
var options = new OptionSet(){{"t|write-runtime-to=",
                               optarg => {if ((runtime_file != null))
                                          ...
                                          runtime_file = new FileStream(optarg,
                                                                        FileMode.Create);}},
                              {"r|runs=", optarg => {num_runs = optarg;
                                                     do_warmup_run = true;}},
                              {"e|entry-point=",
                               optarg => {entry_point = optarg;}},
                              {"b|binary-output", optarg => {}}};
...
class internal
{
    ...
    */
    ...
    (int, byte[], int) futhark_main(long a_mem_sizze_2497, byte[] a_mem_2498,
                                    int sizze_2484)
    {
        var binop_x_2500 = sext_i32_i64(sizze_2484);
        var bytes_2499 = (binop_x_2500 * Convert.ToInt64(4));
        var mem_2501 = allocateMem(bytes_2499);
        var acc_2491 = Convert.ToInt32(0);
        var i_2490 = Convert.ToInt32(0);
        var one_2512 = Convert.ToInt32(1);
        foreach (var counter_2511 in Enumerable.Range(0, sizze_2484))
        {
            var binop_param_y_2488 = indexArray<int>(a_mem_2498,
                                                     (i_2490 * Convert.ToInt32(4)),
                                                     BitConverter.ToInt32);
            var res_2489 = (acc_2491 + binop_param_y_2488);
            writeScalarArray(mem_2501, (i_2490 * Convert.ToInt32(4)),
                             Convert.ToInt32(res_2489));
            var row_2495 = indexArray<int>(mem_2501,
                                           (i_2490 * Convert.ToInt32(4)),
                                           BitConverter.ToInt32);
            var acc_tmp_2509 = row_2495;
            var acc_2491 = acc_tmp_2509;
            i_2490 += one_2512;
        }
        var discard_2496 = acc_2491;
        var out_arrsizze_2508 = sizze_2484;
        var out_memsizze_2507 = bytes_2499;
        var out_mem_2506 = mem_2501;
